...
** Text analysis and generation (ch 12)

- Code along in JupyterLab.

- Only 2 sessions for this chapter (this week + next), so Part 1 (Bag
  of words: unique words, punctuation, word frequencies, dictionary
  subtraction) is what we code along live; Part 2 (random numbers,
  bigrams, Markov analysis, generating text) will go out as the full
  script if we run out of time.
...
* Week 14: Text generation  and Markov analysis (ch 12)
