...
* Week 10 - Escape to the Chapel! (Part 2) / Immutable structures
#+attr_html: :width 700px :float nil:
[[../img/panel.png]]
Remember the CPU panel: next Monday, 5 pm PST
...
- But first we're going to do a *warm-up* exercise to get back into
  dictionaries and dictionaries of dictionaries. We're using the
  scenario that you'll find in the home assignment, "Treasure Island".

- Open this notebook now for the startup exercise:
  [[https://tinyurl.com/treasure-island-ipynb][tinyurl.com/treasure-island-ipynb]]
...
* Week 11: Iteration and search (ch 7)

- Short aside: tuples (~15-20 min). Not covered by DataCamp
  (lists and dictionaries were). Immutable, so usable as dict keys;
  packing/unpacking; the natural type for multiple return values.
  Fold in wherever multiple-return-values comes up naturally, rather
  than as its own block.

- Iteration and linear search (ch 7)
