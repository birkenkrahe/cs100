...
* Week 12: Strings and regular expressions (ch 8)
#+attr_html: :width 400px :float nil:
[[../img/regex.jpeg]]
...
4) Practiced ~str.startswith(prefix, start, end)~ on a filename
   ...
   *Does ~startswith~ have any positional-only arguments?*
   ...
   - Looped over strings three ways: =for letter in s=,
   =range(len(s))=, and =enumerate(s)=.

   *Display "Amen" in these three ways and with a "regular" ~for~ loop.
   #+begin_src python :results output :exports both :session *Python* :python python3
     s = "Amen"
...
5) Classic bug: =if letter == 'e' or 'E'= is always ~True~ (the ASCII
...
6) Modularized the letter-counter into a function, generalized it to
...
7) Started *reading/writing files* with ~open()~ (modes: read/write/
...
- These commands should show you a list of your files:
  #+begin_example python
    import os
    print(os.listdir())
  #+end_example

- The file =quote.txt= should be among them because we created it last
  time.
...
- *Programming assignments:* Pick at least *one* of the exercises
  7.9.2-7.9.6, and *one* of the exercises 8.12.2-8.12.5 and submit your
  solution in a Colab notebook with explanations.

  You can of course solve as many exercises as you like, but one
  (non-AI) exercise from chapter 7 and one from chapter 8 is the
  minimum.

  I will share my sample solutions with you via GitHub.
...
* Week 13: Text analysis and generation (ch 12)

* Week 14: Files and databases (ch 13)
