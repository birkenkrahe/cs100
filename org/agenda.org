...
|-----------------------------------+-----+---|
| Opening prayer                    |   1 |   |
| Post-mortem: ~remove~ vs ~len~ vs ~del~ |  10 |   |
| Projects: Review (proposals)      |  15 |   |
| Conditionals and recursion        |  60 |   |
| Next assignments                  |   5 |   |
| Checkout (takeaway)               |   5 |   |
| Closing prayer                    |   1 |   |
|-----------------------------------+-----+---|
...
[[https://teams.cloud.microsoft/l/message/19:xwJm5Zf90mdM34VuFWBGyX-ofNKhU9JQ7Wbrr55uYSE1@thread.tacv2/1781961912939?tenantId=0a354b0e-92d3-45a0-8b54-df786cb36e0f&groupId=10bc88e3-971f-4b60-ae54-c90368bd6d8c&parentMessageId=1781961912939&teamName=CS%20100&channelName=General%20messages&createdTime=1781961912939][[posted to General Messages, 6/20/2026]​]]

Jim Primbs opened the question by noting that Python lists have
methods like ~.remove()~, but getting the length requires a separate
function, ~len()~, and deleting an element requires the ~del~ statement
followed by the element name — prompting his core question: "Why all
the differences? That seems confusing to me."
...
The short answer: 
#+begin_quote
Python lists have one type-specific method, ~.remove()~, but calls
~len()~, which calls ~list~'s ~__len__~, and a language statement for ~del~,
which calls ~list~'s ~__delitem__~. So the three are different kinds of
construct, not an arbitrary inconsistency but a design feature. The
confusion stems from the layers of abstraction heaped on.
#+end_quote

Here is the longer answer:

1. ~x.remove()~ is a *method*. A function that belongs to a specific type,
   called with dot notation. It only exists because ~list~ and a few
   other containers (like ~set~) chose to write it. There is no general
   Python-wide idea of "remove" that every type shares. It just lives
   where it was defined, as one of ~list~'s own attributes, which is why
   you find ~remove~ sitting in ~dir(list)~ for you to all directly.

2. ~len()~ is a *function that hands the real work off to the object
   itself* (this is also called "dispatch"). It does not know how to
   measure anything. When you call =len(x)=, it asks =x= "How long are
   you?" by calling the hidden method ~__len__~. Every type that wants
   to support ~len~ - ~list~, ~dict~, ~set~ and so on - implements its own
   ~__len__~ that know how to count its own contents. So ~len~ is a thin
   front door (an interface!) while the actual counting logic (the
   implementation!) lives inside each type.

3. =del x[i]= or =del x= is neither a function nor a method but an
   instruction built into the language. It's a keyword like =if= or
   =return= (hence no parentheses). Keywords are recognized by Python
   before your code even runs, not looked up as objects at runtime. So
   when you write =del x[i]=, Python does not go searching for something
   called ~del~ on =x=, but it just knows, from the word itself, what do
   to: quietly turn it into a call to =x='s own hidden ~__delitem__~
   method. That's also why ~del~ can never show up in a ~dir~ list. ~dir~
   lists an object's attributes, and ~del~ is neither an attribute nor
   even an object!

*Why does Python do it this way?*

It's not arbitrary. Python is sorting things into three buckets based
on how widely shared the operation is:
- *Keywords* (~del~) are for things so fundamental to the language that
  they get baked into the grammar itself, like ~if~ or ~for~.
- *Functions* (~len()~) are for operations that almost every container
  wants to support in exactly the same way. Instead of making you
  remember a different way each time, you get one shared function.
- *Methods* (~.remove()~) are for operations that only make sense for some
  types, and often don't even mean the same things across them:
  removing the first match by value has a meaning for ~list~ but not for
  ~dict~ (dictionary).

This was fun to research - thanks for the question - I hope it is a
little clearer now. Programming language construction is a large field
in its own right (and a lot more transparent than natural language
evolution).

*References*

1. Python Software Foundation. [[https://docs.python.org/3/reference/datamodel.html#special-method-names][The Python Language Reference — Data
   Model, §3.3 Special Method Names]]
2. Python Software Foundation. [[https://docs.python.org/3/library/functions.html#len][The Python Standard Library — Built-in
   Functions: len()]]
3. Python Software Foundation. [[https://docs.python.org/3/reference/simple_stmts.html#the-del-statement][The Python Language Reference — Simple
   Statements: The del statement]]
4. Python Software Foundation. [[https://docs.python.org/3/library/functions.html#dir][The Python Standard Library — Built-in
   Functions: dir()]]
5. Python Software Foundation. [[https://docs.python.org/3/tutorial/datastructures.html#more-on-lists][The Python Tutorial — Data Structures:
   More on Lists]]

Further reading: 
- Downey, A. (2024). Think Python, 3rd ed. O'Reilly Media — Ch. 4
  (functions), Ch. 10 (lists).
- Ramalho, L. (2022). Fluent Python, 2nd ed. O'Reilly Media — Ch. 1,
  "The Python Data Model."
- Lutz, M. (2013). Learning Python, 5th ed. O'Reilly Media — Ch. 30,
