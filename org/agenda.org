...
* DONE Week 8 - Function ~return~ values
...
* DONE Week 9 - Dictionaries in Action

** Opening prayer
** New assignments

- Quiz on Control flow and filtering pandas (DataCamp)
- Another DataCamp chapter (loops)

** Review: Dictionary, control flow, and pandas (part 1/2)

- We're going to code a text adventure game "Escape to the chapel"

- Open the starter Colab notebook: [[https://tinyurl.com/escape-to-the-chapel][tinyurl.com/escape-to-the-chapel]]

- You can find the complete solution in GitHub (8_dict_pandas.org)

- Next week: finish the game, turn modular code into functions

** Take aways

- Difference between lookup O(1) and search O(n) (time complexity)
- ~.items()~ function to view all key-value pairs
- Printing sub-dictionaries
- Lots of coding = lots of errors
- Little (elegant, terse) coding = little readability

** Closing prayer

* DONE Week 10 - Escape to the Chapel! (Part 2) / Immutable structures
#+attr_html: :width 700px :float nil:
[[../img/panel.png]]
Remember the CPU panel: next Monday, 5 pm PST - must RSVP
...
- Jim's question: *Mutable* vs *immutable* data types: I've made a video
  on this and uploaded it to MS Teams. You may need this if you want
  to do the refactoring exercise for the "Escape to the Chapel" game.

** New assignments

- New assignments:
  1. New DataCamp lesson: Random walk (case study)
  2. Quiz on Loops (DataCamp) and class content (game)
  3. Treasure Island text adventure game (control flow)
  4. Refactoring of "Escape to the Chapel" (bonus)

** Outlook

- *Outlook*: for the remaining sessions, I will stay close to the
  textbook though you'll continue to get a weekly DataCamp
  session. The next (and last) course is "Introduction to Functions in
  Python", with "Intermediate Python" as its prerequisite.

- *Reading* the respective chapters is *not required* but it might be
  useful to skim them. You don't need to work through the Colab work
  books since we'll do that here in class based on my lectures.

** Code along lecture: Escape to the Chapel II

- First we're going to do a *warm-up* exercise to get back into
  dictionaries and dictionaries of dictionaries. We're using the
  scenario that you'll find in this week's home assignment.

- Open this notebook now for the warm-up exercise:
  [[https://tinyurl.com/treasure-island-ipynb][tinyuNrl.com/treasure-island-ipynb]]

- Today we finish the "Escape to the Chapel" game:
  [[https://tinyurl.com/escape-to-chapel-2][tinyurl.com/escape-to-chapel-2]]

** Your take aways

- What are your *take aways* for today?
  + ~break~ to leave a loop and ~continue~ to take a shortcut back to the
    top
  + Dictionaries (lookup tables) can be nested and linked (=visits=)
  + Loops in a text based game allow you to run it forever (if you
    forgot the =quit= condition).

** TODO Closing prayer

- Let's pray again!

* DONE Week 11: strings and search (ch 7 + 8)
** Opening prayer
[[../img/hail_holy_queen.png]]

** DataCamp review (Intermediate Python)

- You finished another DataCamp course (Intermediate Python)
- Any questions or comments on that course?
- Next up: Introductions to functions in Python

** New assignments

- Quiz based on what we did in class + last DataCamp lesson.
- DataCamp lesson 1 of "Introduction to functions in Python".
- This week's exercise will test your understanding of what we did in
  class directly.

** Strings and search (textbook ch 7 + 8)

- I will code this for you in Jupyterlab
- You can code along or just join me for two practice bits
- What we don't finish today I will record on video

** Take aways

- =import os= gives you access to many Operating System commands
- Learning more about strings
- Positional vs. keyword arguments separation with ~/~ in the function
  docstring
- Characters are encoded as ASCII values (0 to 255)
- Opening and writing to files using data file 'streams'

** Closing prayer
#+attr_html: :width 700px :float nil:
[[../img/memorare.png]]

* DONE Week 12: Strings and regular expressions (ch 8)
#+attr_html: :width 400px :float nil:
[[../img/regex.jpeg]]
...
** Installing Jupyter lab and Python

- See [[https://github.com/birkenkrahe/cs100/blob/main/org/10_jupyterlab_setup.org][instructions in GitHub]]: [[https://tinyurl.com/install-jupyter-lab][tinyurl.com/install-jupyter-lab]]

** Review Last Session / DataCamp "Writing your own functions"
*** Open a Python console in the cloud using Jupyter

*How do you open a Python console in Jupyter*
#+begin_quote
- Open URL: jupyter.org/try-jupyter (or on your PC)
- Open app: ~Console~ > ~Python (Pyodide)~
- Output: ~A WebAssembly-powered Python kernel backed by Pyodide~
#+end_quote

*** Now we review last week's material:

1) We live-coded in *JupyterLab* for the first time (no slides) - strings
   are intuitive, a good fit for a hands-on format.

   *What's different in JupyterLab? Think of at least one thing.*
   #+begin_quote
   + There's no AI assistant available.
   + It's free and open source (not attached to Google).
   + You can install it on your own computer.
   + It runs directly in your browser not on some server on the web.
   #+end_quote

2) *Strings are special*: they have substructure (sequences of
   characters) - explored with indexing and =dir(str)=.

   *How can you display all available methods in ~str~?*
   #+begin_src python :results output :exports both :session *Python* :python python3
     for m in dir(str):
         print(m)
   #+end_src
...
3) Filtered out *dunder* (double underscore) methods with
   ~str.startswith('_'):~ used ~help()~ to read docstrings instead of
   guessing/googling.

   *Filter out the dunder methods from the previous output*:
   #+begin_src python :results output :exports both :session *Python* :python python3
     for m in dir(str):
...
4) Practiced ~str.startswith(prefix, start, end)~ on a filename
   example - noted the ~/~ in the docstring marking positional-only
   arguments.

   *Does ~startswith~ have any positional-only arguments?*
   #+begin_src python :results output :exports both :session *Python* :python python3
     help(str.startswith) # if this shows `self.../` depends on your
...
5) Looped over strings three ways: =for letter in s=,
   =range(len(s))=, and =enumerate(s)=.

   *Display "Amen" in these three ways:
   #+begin_src python :results output :exports both :session *Python* :python python3
     s = "Amen"
     for letter in s: print(letter)
     for letter in range(len(s)): print(s[letter])
     for letter in enumerate(s): print(letter)
   #+end_src
...
6) Classic bug: =if letter == 'e' or 'E'= is always ~True~ (the ASCII
   value of ~'E'~ is non-zero).

   *What is the ASCII value of 'e'?
   #+begin_src python :results output :exports both :session *Python* :python python3
     print(ord('\n')) # returns the ASCII value
     print(repr(chr(10))) # returns the ASCII encoding (character)
   #+end_src
...
7) Modularized the letter-counter into a function, generalized it to
   take any string, then simplified it with the ~in~ operator and
   ~.lower()~.

   *Check if "Amen" has an `e` in it using ~in~ and ~str.lower()~
   #+begin_src python :results output :exports both :session *Python* :python python3
     has_e=False     # logical flag
     for l in "Amen": # loop over the string
         if l.lower() in 'e': # checks for each letter of string if it's an 'e'
             has_e=True # change flag to true
     print(has_e)
   #+end_src
...
8) Started *reading/writing files* with ~open()~ (modes: read/write/
   create), and checking the operating system with ~os.listdir()~.

   *Write "hello" to file =hello.txt=, then check if it was created.
   #+begin_src python :results output :exports both :session *Python* :python python3
...
*** Now we review the DataCamp lesson "Write your own functions"

1) What does this program print? Why?
   #+begin_src python :results output :exports both :session *Python* :python python3
     def add(a,b):
         """Add a and b"""
         return a + b

     result = print(add(2,3))  # prints 5
     print(result)  # prints nothing (None) 
   #+end_src
   ...
   #+begin_quote
   ~print()~ displays, it does not ~return~ a value. It always returns
   ~None~. The first call prints the result of =add(2,3)=, which is =5=, then
   stores the ~return~ value of =print(5)= in =result=, which is ~None~.
   #+end_quote

2) What are =foo= and =bar=? What's the difference between them?
   #+begin_src python :results output :exports both :session *Python* :python python3
     foo = [1,2,3]  # list
     bar = (1,2,3)  # tuple
     print(foo)
   ...
   #+begin_quote
   =foo= is a list - mutable, =bar= is a tuple - immutable. This is why
   tuples can be used as dictionary keys, but lists cannot!
   #+end_quote
   #+begin_src python :results output :exports both :session *Python* :python python3
     foo[1]=100
     print(foo)
     bar[1]=100 # TypeError: 'tuple' object does not support item assignment
   #+end_src

   #+RESULTS:
   : [1, 100, 3]

3) Write a function =swap= that swaps two strings and returns *both* of
   them.
   #+begin_src python :results output :exports both :session *Python* :python python3
     def swap(s1,s2):
         """Swap s1 and s2"""
         ...
         s1 = s2
         s2 = t
         return (s1,s2)   # return two strings (packed in a tuple)

     foo='hello'
     bar='goodbye'
     print(f'{foo} and {bar}')  # hello goodbye
     foo,bar = swap('hello','goodbye')
     print(f'{foo} and {bar}')  # goodbye hello
   #+end_src
...
4) *Can you ~return~ a ~list~, a NumPy array, or a dictionary from a
   function?*
   #+begin_src python :results output :exports both :session *Python* :python python3
     def list_return(l):
         l.append("to that")
   ...
   #+begin_src python :results output :exports both :session *Python* :python python3
     ...
     def array_return(a):
         a=a*2   # cannot do this with lists
         return a

     arr = np.array([100,200]) # input: [100 200]
     result = array_return(arr)
     print(result) # output: [200 400]
   #+end_src
   ...
   #+begin_src python :results output :exports both :session *Python* :python python3 
     def dict_return(d):
         None_dict = {"None":"Represents no value, of NoneType"}
         d = None_dict
         return d

     defs = { "dictionary":"A key-value data structure",
              "array":"An ordered set of values of one data type",
              "list":"An ordered set of values of any data type"
             }
     print(defs)
     result = dict_return(defs)
     print(result)
   #+end_src

   #+RESULTS:
   : {'dictionary': 'A key-value data structure', 'array': 'An ordered set of values of one data type', 'list': 'An ordered set of values of any data type'}
   : {'None': 'Represents no value, of NoneType'}
...
- Use regular expressions to find and replace strings in files

To continue last week's session:

- https://jupyter.org/try-jupyter/lab/
- In Jupyter, click on ~+~ to open the ~Launcher~ and choose ~Notebook~ >
  ~Python (Pyodide)~

- Code along with me in Jupyter.

** New assignments

- *Programming assignments:* ???

- *Quiz* covering DataCamp (1st lesson - writing functions in Python)
  and today's classroom session.

- *DataCamp lesson*: default arguments, variable-length arguments, and
  scope in functions.

** Take aways

- Finding and downloading books into files Python can work with
- Stripping strings with ~str.strip()~
- Files must be ~.closed~ when you're done with them
- Writing into a text file through code
- In Python you can return a list (and an array, a tuple, a dict)
...
* DONE Week 13: Text cleaning and bag-of-words analysis (ch 12)
** Opening prayer
...
- One last DataCamp assignment: Lambda functions and error handling

- [Last] Quiz on today's material and the last DataCamp session

- *Next week*: One [optional] programming challenge: clean a corpus from
  stop-words. Here is a starter notebook if you want to try it. Run
  the setup first then fill in the rest:
  [[https://tinyurl.com/text-analysis-challenge-ipynb][tinyurl.com/text-analysis-challenge-ipynb]]

** Corpus creation and text analysis

- Code along in JupyterLab.

- Part 1: *Bag-of-words* - cleaning the data to create a text *corpus* and
  doing some preliminary *text analysis*, both in text and in graphs.

- Part 2: *Markov chain* text generation: injecting *randomness*,
  generating *bigrams*, creating a *lookup table* and generate text.

** Take aways

- Suppress ~None~ from ~print()~ with a closing semi-colon
- Unicode database connection to categorize characters
- Computing word frequencies from a cleaned text corpus
...
* Week 14: Markov analysis and text generation
** Opening prayer
...
1) How can you create a dictionary of unique words from a text?
   #+begin_example
   1) Create an empty dictionary
     2) Go through every line of the file
       3) Split every line into words separated by spaces
         4) Go through every word of the line
           5) Add the word as a key to the dictionary
   #+end_example
2) What's a list comprehension?
   #+begin_quote
   A way to build a list by looping and filtering in a single
   expression instead of writing a ~for~ loop with ~.append()~:

   [ =expression= ~for~ =item= ~in~ =iterable= ]
   #+end_quote
3) Example: filter out all even/odd numbers in [0,10)
   #+begin_src python :results output :exports both :session *Python* :python python3 
     [print(x) for x in range(10) if x % 2 == 0] # even numbers in [0,10)
   #+end_src
4) How can you filter out unique words sorted by word length?
   #+begin_example
   [print(w) for w in sorted(unique_words, key=len)[-10:-4]];
   #+end_example
5) In the last answer, what does =[-10:-4]= mean?
   #+begin_quote
   Print 10-4=6 elements from the end of the list and leave out the
   last three.
   #+end_quote
6) Which string functions were used to get rid of punctuation marks?
   #+begin_quote
   - ~.replace(old,new)~
   - ~.split()~
   - ~.strip()~
   - ~.lower()~
   #+end_quote
7) What is ~unicodedata~?
   #+begin_quote
   A module that provides access to the Unicode Character
   Database. ~unicodedata.category~ returns a category string, e.g. ~Po~
   for punctuation, or ~Lu~ for uppercase letters.
   #+end_quote
8) How can you compute word frequencies from a clean corpus of words?
   #+begin_example
   1) Create an empty dictionary
     2) Go through every line of the file
       3) Go through every word in the line (no --)
         4) Clean the word (no punctuation, lower case)
           5) If word was not seen yet -> assign 1
           6) If word was seen before  -> add 1
   #+end_example
9) We want to sort by the second element (the frequency) of the
   dictionary. Why don't we just use ~word_counter.values()~?
   #+begin_example
   items = sorted(word_counter.items(),    # contains key + value
                  key=second_element,      # second_element returns value
                  reverse=True)
   #+end_example
10) Can you explain this command?
    #+begin_example
    [print(f"{word:<5}{freq}") for word,freq in items[:5]];
    #+end_example
    #+begin_quote
    1. list comprehension (returns a list)
    2. prints with format string
    3. prints two variables, `word` and `freq`
    4. prints `word` on 5 spaces
    5. loops over the list `items` which consists of pairs (2-tuple)
    6. unpacks `word` and `freq` from each pair
    7. extracts five elements with indices 0-4 from `items`
    #+end_quote
    Same as:
    #+begin_example
    for word, freq in items[:5]:
        print(f"{word:<5}{freq}")
    #+end_example
** Continue text analysis and text generation
- [[https://raw.githubusercontent.com/birkenkrahe/cs100/refs/heads/main/ipynb/text_analysis_august_14.ipynb][Upload this notebook from URL to Jupyter.org/try-jupyter/lab]]

- This is the current status of our notebook

- Go to *Edit > Clear output of all cells*

- Let's go through what we did and continue the exercise
...
* Week 15: Final presentations of your projects
